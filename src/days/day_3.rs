pub mod problem_1 {
    ...
    pub fn answer() {
        let input = read_inputs("problems/day_3.txt");
        let most_common = find_most_common(&input);
        let least_common = !most_common & 0xFFF;
        println!("Most common bytes are {:#012b}, {}", most_common, most_common);
        println!("Least common bytes are {:#012b}, {}", least_common, least_common);
        println!("The total power is {}", (most_common as u32) * (least_common as u32));
    }
}
...
pub mod problem_2 {
    ...
    fn count_ones(values: &Vec<u16>, pos: u16) -> i32 {
        ...
        values
            .iter()
            .fold(0, move |count, val| {
                if val & mask != 0 { count + 1 } else { count }
            })
    }
    ...
    fn oxygen_generator_rating(values: &Vec<u16>) -> u16 {
        ...
        while filtered.len() > 1 {
            ...
            println!("{} number of {} have ones at position {}", num_ones, filtered.len(), pos);
    ...
    pub fn answer() {
        let inputs = read_inputs("problems/day_3.txt");
